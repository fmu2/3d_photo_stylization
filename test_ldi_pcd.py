...
def main(args):
    ...
    if args.plot:
        # down-sample
        assert args.down_sample >= 1, \
            '[ERROR] point cloud down-sampling factor must be at least 1'
        if args.down_sample > 1:
            idx = random.sample(range(n_pts), int(n_pts / args.down_sample))
            xyz, rgb = xyz[idx], rgb[idx]

        # convert to NDC space
        xyz = torch.from_numpy(xyz[None])
        z = xyz[..., 2]
        near = 0.99 * z.amin(1)
        ...
        plot_point_cloud(xyz, rgb, save_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-plot', '--plot', action='store_true', default=False,
                        help='if True, plot point cloud in NDC space')
    parser.add_argument('-ds', '--down_sample', type=float, default=1.0,
                        help='point cloud down-sampling factor')
